...
$("#next-btn2").click(function() {
	...
	$("#fieldset2").find(".sched-days").each(function(index) {
		var days_selected = 0;
	...
	$("#fieldset2").find(".sched-to").each(function(index) {
		var from = $($("#fieldset2").find(".sched-from").get(index)).val();
		if ($(this).val() <= from) {
...
function add_class(results) {
	...
	if (rows_error.length == 0 || rows_error[0] == data.length) {
		alert("All " + data.length-2 + " users added successfully.");
	} else {
