$(document).ready(function() {
	$.ajax({
		...
		success: function(data) {
			...
			for (var key in data) {
				...
				datapoints_pie.push({y: value, name: key, percentage: ((value/total)*100).toFixed(2)});
				datapoints_bar.push({y: value, label: key});
			}
			var chart_pie = new CanvasJS.Chart('pie-population', {
				...
				data: [{
					type: "pie",
					showInLegend: true,
					toopTipContent: "{name}: <strong>{y}%{/strong}",
					indexLabel: "{name} - {percentage}%",
					dataPoints: datapoints_pie,
				}]
			});
			var chart_bar = new CanvasJS.Chart('bar-population', {
				animationEnabled: true,
				title: {
					text: "Population",
				},
	...
	$.ajax({
		...
		success: function(data) {
			...
			var chart = new CanvasJS.Chart('pie-multidisciplinary', {
				animationEnabled: true,
				title: {
	...
	$('#all_engagement').DataTable({
		dom: "frtipB",
	    buttons: [
	      	{extend: 'excel', text: 'Export', filename: "Techno Engagements", className: 'button button1'},
	    ],
	    order: [[ 0, "desc" ]],
